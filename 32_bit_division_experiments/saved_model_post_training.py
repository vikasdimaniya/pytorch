"""
Post-training: retrain h=256 model (same 3-phase strategy), save checkpoint,
then continue training with more epochs at LR=0.000104.
"""
import torch
...
# ============================================================
# PHASE 1: Quick curriculum (same as before)
# ============================================================
...
# ============================================================
# PHASE 2: Cyclic LR on mixed data (same as before)
# ============================================================
phase2_epochs = 150
print(f"\nPHASE 2: Cyclic LR ({phase2_epochs} epochs)")
...
for ep in range(1, phase2_epochs + 1):
    ...
    if ep % 10 == 0:
        ...
        if exact_acc > best_exact:
            ...
            saved = " *** SAVED ***"
        print(f"  ep {ep}/{phase2_epochs} (g{global_epoch}) [{elapsed:.0f}s] bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)


# ============================================================
# PHASE 3: Cosine fine-tune (same as before)
# ============================================================
phase3_epochs = 100
...
for ep in range(1, phase3_epochs + 1):
    ...
    if ep % 10 == 0:
        ...
        if exact_acc > best_exact:
            ...
            torch.save(model.state_dict(), CHECKPOINT_PATH)
            saved = " *** SAVED ***"
        print(f"  ep {ep}/{phase3_epochs} (g{global_epoch}) [{elapsed:.0f}s] bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)
...
bit_acc, exact_acc = evaluate(model, test_loader)
best_exact = max(best_exact, exact_acc)
print(f"\n--- Phases 1-3 complete [{elapsed:.0f}s] ---")
print(f"Best so far: {best_exact:.2f}%")


# ============================================================
# PHASE 4: POST-TRAINING — continue from best model at LR=0.000104
# ============================================================
phase4_epochs = 200
post_lr = 0.000104
print(f"\n{'='*60}")
print(f"PHASE 4: Post-training from best model, LR={post_lr}, {phase4_epochs} epochs")
print(f"{'='*60}")

model.load_state_dict(torch.load(CHECKPOINT_PATH, weights_only=True))
print(f"  Loaded best checkpoint (exact={best_exact:.2f}%)")

optimizer = torch.optim.Adam(model.parameters(), lr=post_lr)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer, T_max=phase4_epochs, eta_min=1e-6
)
...
for ep in range(1, phase4_epochs + 1):
    ...
    if ep % 5 == 0:
        ...
        if exact_acc > best_exact:
            ...
            torch.save(model.state_dict(), CHECKPOINT_PATH)
            saved = " *** SAVED ***"
        print(f"  ep {ep}/{phase4_epochs} (g{global_epoch}) [{elapsed:.0f}s] lr={lr_now:.6f} bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)

train_time = time.time() - start_time
model.load_state_dict(torch.load(CHECKPOINT_PATH, weights_only=True))
bit_acc, exact_acc = evaluate(model, test_loader)
print(f"\nFinal (best model): bit={bit_acc:.2f}%, exact={exact_acc:.2f}%")
print(f"RESULT|GRU-S2S-PostTrain|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.2f}")
print(f"\nCheckpoint saved at: {os.path.abspath(CHECKPOINT_PATH)}")
