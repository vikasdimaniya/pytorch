"""
Resume standard h=512 GRU from best_2hr_standard.pt (33.83%).
Another hour of cyclic + cosine fine-tune.
"""
import torch
...
BATCH_SIZE = 512
TIME_LIMIT = 3600
CKPT_IN = "best_3hr_standard.pt"
CKPT_OUT = "best_3hr_standard.pt"
...
# ============================================================
print(f"{'='*60}")
print(f"RESUME v2: Standard GRU h={hidden_size} from {CKPT_IN}")
print(f"{'='*60}\n")
...
# Fresh data again (yet another seed)
train_pairs = sample_smart(n_per_qlen_base=2000, max_q_bits=10, seed=456)
train_loader = DataLoader(DivisionDataset(train_pairs), batch_size=BATCH_SIZE, shuffle=True)
print(f"Train: {len(train_pairs)} samples (fresh seed=456)")
...
print(f"RESULT|Resume2-Standard-h512|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.0f}")
