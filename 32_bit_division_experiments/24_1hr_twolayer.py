"""
1-hour training: 2-layer GRU h=512 with dropout.
Architectural tweak: deeper GRU should help the carry chain
by giving a second layer to refine hidden states.
...
class GRU2LayerSeq2Seq(nn.Module):
    """2-layer GRU with dropout between layers.

    Layer 1 processes raw input features → hidden representations.
    Layer 2 refines those representations → better carry/remainder tracking.
    Dropout (0.1) between layers prevents co-adaptation.
    """
    def __init__(self):
        super().__init__()
        self.hidden_size = hidden_size
        self.gru = nn.GRU(33, hidden_size, num_layers=2, batch_first=True, dropout=0.1)
        self.fc = nn.Linear(hidden_size, 1)

    def forward(self, x):
        h0 = torch.zeros(2, x.size(0), self.hidden_size, device=x.device)
        out, _ = self.gru(x, h0)
...
# ============================================================
print(f"{'='*60}")
print(f"1-HOUR RUN: 2-Layer GRU h={hidden_size} + dropout")
print(f"{'='*60}\n")
...
model = GRU2LayerSeq2Seq().to(device)
n_params = sum(p.numel() for p in model.parameters())
print(f"Model: {n_params:,} params (2-layer GRU + dropout=0.1)")
...
print(f"RESULT|2Layer-h512|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.0f}")
