"""
Resume training standard h=512 GRU from best checkpoint.
Cyclic LR to explore + cosine to fine-tune for 1 more hour.
"""
import torch
...
BATCH_SIZE = 512
TIME_LIMIT = 3600
CKPT_IN = "best_1hr_standard.pt"
CKPT_OUT = "best_2hr_standard.pt"
...
# ============================================================
print(f"{'='*60}")
print(f"RESUME: Standard GRU h={hidden_size} from {CKPT_IN}")
print(f"{'='*60}\n")

# Same test set as before for fair comparison
...
# Fresh training data (different seed to avoid memorization)
train_pairs = sample_smart(n_per_qlen_base=2000, max_q_bits=10, seed=123)
train_loader = DataLoader(DivisionDataset(train_pairs), batch_size=BATCH_SIZE, shuffle=True)
print(f"Train: {len(train_pairs)} samples (fresh seed)")
print(f"Test: {len(test_pairs)} samples\n")

model = GRUSeq2Seq().to(device)
model.load_state_dict(torch.load(CKPT_IN, weights_only=True))
n_params = sum(p.numel() for p in model.parameters())
print(f"Model: {n_params:,} params — loaded from checkpoint")

bit_acc, exact_acc = evaluate(model, test_loader)
best_exact = exact_acc
print(f"Starting accuracy: bit={bit_acc:.2f}%, exact={exact_acc:.2f}%\n")
...
# ============================================================
# PHASE A: Cyclic LR exploration (60% of time)
# ============================================================
phaseA_end = TIME_LIMIT * 0.60
print(f"PHASE A: Cyclic LR exploration")
steps_per_epoch = len(train_loader)
optimizer = torch.optim.Adam(model.parameters(), lr=0.00005)
scheduler = torch.optim.lr_scheduler.CyclicLR(
    optimizer, base_lr=0.00005, max_lr=0.001,
    step_size_up=steps_per_epoch * 10, step_size_down=steps_per_epoch * 10,
    mode='triangular2', cycle_momentum=False,
)

ep = 0
while (time.time() - start_time) < phaseA_end:
    ep += 1
    global_epoch += 1
    model.train()
    ...
    if ep % 5 == 0:
        ...
        print(f"  ep {ep} [{elapsed:.0f}s] lr={lr:.6f} bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)

print(f"  Phase A done: {ep} epochs\n")


# ============================================================
# PHASE B: Cosine fine-tune from best (remaining time)
# ============================================================
remaining = time_left()
print(f"PHASE B: Cosine fine-tune ({remaining:.0f}s remaining)")
model.load_state_dict(torch.load(CKPT_OUT, weights_only=True))
...
while time_left() > 20:
    ...
    if ep % 5 == 0:
        ...
        if exact_acc > best_exact:
            ...
            torch.save(model.state_dict(), CKPT_OUT)
            saved = " *** SAVED ***"
        print(f"  ep {ep} [{elapsed:.0f}s] bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)

print(f"  Phase B done: {ep} epochs\n")
...
bit_acc, exact_acc = evaluate(model, test_loader)
print(f"Final: bit={bit_acc:.2f}%, exact={exact_acc:.2f}% [{train_time:.0f}s] ({global_epoch} epochs)")
print(f"RESULT|Resume-Standard-h512|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.0f}")
...
print(f"\nCheckpoint saved: {os.path.abspath(CKPT_OUT)}")
