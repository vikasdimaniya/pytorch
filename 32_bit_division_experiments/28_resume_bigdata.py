"""
Resume bigger-data h=512 GRU from best_bigdata.pt (21.61%).
Another hour of cyclic + cosine fine-tune with 40K data.
"""
import torch
...
BATCH_SIZE = 512
TIME_LIMIT = 3600
CKPT_IN = "best_bigdata_v2.pt"
CKPT_OUT = "best_bigdata_v3.pt"
...
def sample_smart(n_per_qlen_base, max_q_bits, seed):
    weights = {1: 1, 2: 1, 3: 3, 4: 3, 5: 3, 6: 2, 7: 2, 8: 2, 9: 1, 10: 1}
...
# ============================================================
print(f"{'='*60}")
print(f"RESUME: BigData GRU h={hidden_size} from {CKPT_IN}")
print(f"{'='*60}\n")
...
# Same heavy Q=3-5 oversampling, fresh seed
train_pairs = sample_smart(n_per_qlen_base=2100, max_q_bits=10, seed=789)
train_loader = DataLoader(DivisionDataset(train_pairs), batch_size=BATCH_SIZE, shuffle=True)
print(f"Train: {len(train_pairs)} samples (fresh seed=789)")
...
# ============================================================
# PHASE B: Cosine fine-tune (remaining time)
# ============================================================
remaining = time_left()
...
print(f"RESULT|Resume-BigData-h512|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.0f}")
