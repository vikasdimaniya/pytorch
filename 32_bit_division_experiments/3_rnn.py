...
def reshape_for_rnn(x):
    # x: (batch, 64) -> (batch, 32, 2): 32 steps, each [a_bit, b_bit]
    bits_a = x[:, :N_BITS]       # (batch, 32)
    bits_b = x[:, N_BITS:]       # (batch, 32)
    a_expanded = bits_a.unsqueeze(2)  # (batch, 32, 1)
    b_expanded = bits_b.unsqueeze(2)  # (batch, 32, 1)
    return torch.cat([a_expanded, b_expanded], dim=2)  # (batch, 32, 2)
