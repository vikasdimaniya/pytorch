...
def reshape_for_seq2seq(x):
    """Reshape to (batch, 32, 2) with MSB first — natural order for division."""
    bits_a = x[:, :N_BITS].flip(1)   # flip LSB-first to MSB-first
...
class RNNSeq2Seq(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.rnn(x, h0)        # (batch, 32, hidden)
        bits = self.fc(out).squeeze(2)   # (batch, 32) — MSB-first quotient bits
        return bits
...
def train_evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            y_msb = y.flip(1)  # target also MSB-first
            outputs = model(x_seq)
