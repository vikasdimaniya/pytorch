"""
32-bit division with hidden_size=256.
Same 3-phase strategy as v18 (curriculum + cyclic LR + cosine fine-tune).
"""
import torch
...
# PHASE 2: Cyclic LR on mixed data
phase2_epochs = 150
...
for ep in range(1, phase2_epochs + 1):
    ...
    if ep % 5 == 0:
        ...
        if exact_acc > best_exact:
            ...
            saved = " *** SAVED ***"
        print(f"  ep {ep}/{phase2_epochs} (g{global_epoch}) [{elapsed:.0f}s] lr={lr_now:.6f} bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)
...
for ep in range(1, phase3_epochs + 1):
    ...
    if ep % 5 == 0:
        ...
        if exact_acc > best_exact:
            ...
            torch.save(model.state_dict(), CHECKPOINT_PATH)
            saved = " *** SAVED ***"
        print(f"  ep {ep}/{phase3_epochs} (g{global_epoch}) [{elapsed:.0f}s] lr={lr_now:.6f} bit={bit_acc:.2f}% exact={exact_acc:.2f}%{saved} (best={best_exact:.2f}%)", flush=True)

train_time = time.time() - start_time
model.load_state_dict(torch.load(CHECKPOINT_PATH, weights_only=True))
bit_acc, exact_acc = evaluate(model, test_loader)
print(f"\nFinal (best model): bit={bit_acc:.2f}%, exact={exact_acc:.2f}%")
print(f"RESULT|GRU-S2S-h256|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.2f}")
