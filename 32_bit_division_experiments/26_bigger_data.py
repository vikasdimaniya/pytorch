"""
Fresh h=512 GRU trained with 2x more data (40K) and heavier
hard-case oversampling. Tests whether data volume + distribution
matters more than training time.
"""
import torch
...
# ============================================================
print(f"{'='*60}")
print(f"BIGGER DATA: GRU h={hidden_size}, 40K training samples")
print(f"{'='*60}\n")

# 40K training data with heavy Q=3-5 oversampling
train_pairs = sample_smart(n_per_qlen_base=2100, max_q_bits=10, seed=77)
...
for stage_bits, stage_epochs, n_samp in [(2, 5, 2000), (4, 5, 3000), (8, 8, 6000), (16, 8, 10000)]:
    if time_left() < TIME_LIMIT * 0.85:
        break
    loader = DataLoader(
        DivisionDataset(sample_curriculum(stage_bits, n_samp, seed=77)),
...
# ============================================================
# PHASE 2: Cyclic LR (55% of time)
# ============================================================
phase2_end = TIME_LIMIT * 0.65
print(f"\nPHASE 2: Cyclic LR on big balanced data")
steps_per_epoch = len(train_loader)
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
scheduler = torch.optim.lr_scheduler.CyclicLR(
    optimizer, base_lr=0.0001, max_lr=0.002,
    step_size_up=steps_per_epoch * 12, step_size_down=steps_per_epoch * 12,
...
# ============================================================
# PHASE 3: Cosine fine-tune (remaining)
# ============================================================
remaining = time_left()
print(f"PHASE 3: Cosine fine-tune ({remaining:.0f}s remaining)")
...
print(f"RESULT|BigData-h512|{n_params}|{bit_acc:.2f}|{exact_acc:.2f}|{train_time:.0f}")
